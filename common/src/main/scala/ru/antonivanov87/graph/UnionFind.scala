...
class UnionFind(
  private[graph] val elem2Parent: Map[Int, Int],
  private[graph] val elem2Size: Map[Int, Int]) {
  ...
  private[graph] def compress(elem: Int): UnionFind = {
  ...
  private[graph] def getElemsToCompress(elem: Int): Map[Int, Int] = {
